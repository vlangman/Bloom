...
// settings
const int SCR_WIDTH = 2;
const int SCR_HEIGHT = 2;



void SpeciesInitializer(std::vector<Species>& speciesList)
{

  
    GeneticFactory factory;
    factory.GenerateChromosomes(16);
...
void CreateBuffers(std::vector<Species> &speciesList ,GLuint AgentBuffer, int speciesCount)
{
    ...
    SpeciesInitializer(speciesList);
...
int main()
{
   ...
   /* CONNECTING[LEFT_LINE_SENSOR] TO[INTERNAL_2]
        CONNECTING[INTERNAL_0] TO[TURN_TARGET_DIRECTION_LEFT]
        CONNECTING[RIGHT_LINE_SENSOR] TO[TURN_TARGET_DIRECTION_RIGHT]
        CONNECTING[INTERNAL_0] TO[INTERNAL_1]*/

 /*   test[LEFT_LINE_SENSOR] = std::vector<NEURON_TYPES>{
     INTERNAL_2,
    };
    ...
    test[RIGHT_LINE_SENSOR] = std::vector<NEURON_TYPES>{
        TURN_TARGET_DIRECTION_RIGHT,
    };
    ...
    GLint resPs = glGetUniformLocation(computeProgram, "resolution");
