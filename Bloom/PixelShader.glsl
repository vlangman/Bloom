    ...
    for(int offsetX =-sensSize; offsetX <= sensSize; offsetX++)
    {
        for(int offsetY =-sensSize; offsetY <= sensSize; offsetY++)
        {
            ...
            if(pos.x >= 0 && pos.x < resolution.x && pos.y >= 0 && pos.y < resolution.y){
                ...
                int SpeciesID =  int(trail_buffer.trailpixels[uint(pos.y * resolution.x + pos.x)].SpeciesID_evapSpeed_blendSpeed.x);
                float alpha = trail_buffer.trailpixels[uint(pos.y * resolution.x + pos.x)].pixelColour.a;

                //if (FOLLOW_SELF ) only add the alpha of species == species ID or black space
                if(speciesIn.trailBehaviour.x == FOLLOW_SELF && (speciesIn.active_blendSize_sensorSize_speciesIndex.w == SpeciesID ) )
                {
                      sum += alpha;
                      samples++;
                }
...
//      speciesIn.angle_speed_turnSpeed.x += (randomSeed - 0.5) * 2 *  speciesIn.angle_speed_turnSpeed.z * deltaTime;
...
void main() 
{
  ...
  species = SelectDirection( species,hashFunc(float(index)));
  ...
  float Random = hashFunc(float(newPos.y * resolution.x + newPos.x));
  if(newPos.x < 0 || newPos.x >= resolution.x - 1 || newPos.y < 0 || newPos.y >= resolution.y - 1)
  {
    newPos.x = min( max(2,newPos.x) , resolution.x-1);
    newPos.y = min( max(2,newPos.y) , resolution.y-1);
...
//  species_in.species[index].evapSpeed_blendSpeed_sensOffsetDist_senseAngleSpacing.z -= 0.1* deltaTime;
